...
3. **港澳台**（香港、澳门、台湾）：
   ...
   - **香港特别行政区**：层级为 特区 → 区域（香港岛/九龙/新界）→ 区（中西区、湾仔区等），无街道级
   - **澳门特别行政区**：层级为 特区 → 区域（澳门半岛/澳门外岛）→ 堂区，无街道级
   - **台湾省**：层级为 省 → 市/县 → 区/乡镇，与大陆「省→市→区→街道」类似但最下级为区或乡镇
...
3. **链接与路径**：
   ...
   - 链接形式：`{地区拼音}/{产品页}.html`，例如：
     - `xianggang/cloud.html`、`aomen/cloud.html`、`taiwan/cloud.html`
     - `xianggang/ddos.html`、`aomen/ddos.html`、`taiwan/ddos.html`
   - 生成索引时，港澳台区块的 HTML 结构可与直辖市一致（左侧「港澳台：」，右侧三个链接）。

4. **数据与层级**（若生成港澳台下级页面）：
   - 香港：`香港特别行政区` → 区域（香港岛/九龙/新界）→ 区列表（无街道）
   - 澳门：`澳门特别行政区` → 区域（澳门半岛/澳门外岛）→ 堂区列表（无街道）
   - 台湾：`台湾省` → 市/县 → 区/乡镇（层级与大陆省→市→区类似，最下级为区或乡镇）
   - 港澳台若使用单独数据文件（如 `HK-MO-TW.json`），需在生成流程中合并或单独遍历后，统一输出到 `output/xianggang/`、`output/aomen/`、`output/taiwan/` 下。

5. **实现要点**：
   - 首页索引生成时，除按大陆「直辖市 + 七大区域省份」分组外，显式增加「港澳台」分组，并输出香港、澳门、台湾三项及对应拼音与链接。
   - 拼音固定为：香港 → `xianggang`，澳门 → `aomen`，台湾 → `taiwan`（与模板一致，避免 pypinyin 多音或习惯写法差异）。
