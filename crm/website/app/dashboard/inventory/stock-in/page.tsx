...
export default function StockInPage() {
  ...
  const fetchStockInRecords = async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      const response = await fetch(`/api/inventory/transactions?type=INBOUND&page=${page}&limit=10`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
      const data = await response.json();
      if (data.success) {
        setStockInRecords(data.data.transactions);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setSnackbar({ open: true, message: data.message || '入库失败', severity: 'error' });
      }
    } catch (error) {
      console.error('入库操作失败:', error);
      setSnackbar({ open: true, message: '入库操作失败', severity: 'error' });
    }
  };
